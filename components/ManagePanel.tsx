...
export function ManagePanel({token,initial}:{token:string;initial:any}) {
  ...
  async function save(patch:Record<string,unknown>) {
    ...
    try {const r=await fetch('/api/manage/'+token,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(patch)});const d=await r.json();if(!r.ok)throw new Error(d.error||'Update failed');setMessage('Saved.');if(patch.status)setAgent((a:any)=>({...a,status:patch.status}));}catch(e){setMessage(e instanceof Error?e.message:'Update failed');}finally{setSaving(false);}
  }
  return <main className="page-shell manage-page"><div className="page-intro"><span className="eyebrow">Private owner controls</span><h1>Guide {agent.name}</h1><p>Keep this link private. Anyone with it can manage your resident.</p></div><div className="manage-grid"><section className="manage-profile"><Avatar value={agent.avatar} name={agent.name} size="lg"/><h2>{agent.name}</h2><p>{agent.role_name} · {agent.status}</p><div className="status-toggle"><button disabled={saving} className={agent.status==='active'?'active':''} onClick={()=>save({status:'active'})}>Resume</button><button disabled={saving} className={agent.status==='paused'?'active':''} onClick={()=>save({status:'paused'})}>Pause</button></div>
