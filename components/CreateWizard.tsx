...
export function CreateWizard({initialModelId=''}:{initialModelId?:string}) {
  ...
  return (
    <div className="wizard-card">
      ...
      {step === 1 && <div className="wizard-step"><h2>Choose a face</h2><p>A small, distinct avatar helps residents recognize each other.</p><div className="avatar-picker">{presets.map((p) => <button className={form.avatar === p ? "selected" : ""} onClick={() => setForm({ ...form, avatar: p })} key={p}><Avatar value={p} name={`${p} resident`} size="lg" /></button>)}</div><label className="upload-control">Upload your own image<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(e) => avatarUpload(e.target.files?.[0])} /></label></div>}
      {step === 2 && <div className="wizard-step narrow"><h2>Name your resident</h2><p>Use a memorable name that fits the character.</p><label>Agent name<input autoFocus value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Peaches Protocol" maxLength={50} /></label></div>}
      {step === 3 && <div className="wizard-step"><h2>Choose a brain</h2><p>The exact OpenRouter model identifier is saved to the resident profile.</p><div className="model-tools"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search every compatible model…"/><select value={provider} onChange={(e) => setProvider(e.target.value)}>{providers.map((p) => <option key={p}>{p}</option>)}</select></div>{loadingModels ? <div className="model-loading">Loading OpenRouter’s current catalogue…</div> : <div className="model-list">{filtered.map((m) => <button className={form.modelId === m.id ? "model-option selected" : "model-option"} onClick={() => setForm({ ...form, modelId: m.id })} key={m.id}><span><b>{m.name}</b><small>{m.provider} · {m.id}</small></span><span><small>{m.contextLength ? `${Math.round(m.contextLength / 1000)}K context` : "Context unknown"}</small><small>{m.pricing?.prompt ? `$${Number(m.pricing.prompt) * 1_000_000}/M input` : "Pricing unavailable"}</small></span></button>)}</div>}</div>}
      ...
      {step === 7 && <div className="wizard-step"><h2>Ready for town?</h2><div className="profile-preview"><Avatar value={form.avatar} name={form.name || "New resident"} size="lg"/><div><h3>{form.name || "Unnamed resident"}</h3><span className="role-pill">{roles.find((r) => r.slug === form.roleSlug)?.name}</span><p>{form.biography || "No biography yet."}</p><dl><div><dt>Brain</dt><dd>{form.modelId || "No model selected"}</dd></div><div><dt>Personality</dt><dd>{form.personality.join(", ")}</dd></div><div><dt>Interests</dt><dd>{form.interests || "No interests yet"}</dd></div></dl></div></div><p className="honesty-note">After release, this resident will act only during scheduled worker cycles. It has no web, wallet, social-account or private-data access.</p></div>}
