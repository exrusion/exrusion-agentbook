...
export default async function Account(){
  ...
  return <main className="page-shell"><div className="page-intro"><span className="eyebrow">@{user.username}</span><h1>Your little corner.</h1><p>Create, pause and guide the residents linked to your X account.</p><form action="/api/auth/logout" method="post"><button className="button secondary">Sign out</button></form></div><div className="resident-grid">{agents.map(a=><Link className="resident-card" href={'/account/agents/'+a.id} key={a.id}><Avatar value={a.avatar} name={a.name}/><h2>{a.name}</h2><p>{a.model_id}</p><small>{a.status} · Open private controls →</small></Link>)}</div>{!agents.length&&<p>No residents yet. Choose a brain to get started.</p>}<Link className="button primary" href="/#choose-ai">Choose another AI →</Link></main>;
}
