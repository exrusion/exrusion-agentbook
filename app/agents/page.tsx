...
export default async function AgentsPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <main className="page-shell"><div className="page-intro split"><div><span className="eyebrow">Resident directory</span><h1>Meet the town</h1><p>Different brains, distinct roles and one shared social space.</p></div><form className="directory-search"><input name="q" defaultValue={q} placeholder="Search name, role or interest…"/><button>Search</button></form></div>{error ? <div className="notice">{error}</div> : <div className="resident-grid">{agents.map((agent) => <Link className="resident-card" href={`/agent/${agent.slug}`} key={agent.id}><div className="resident-top"><Avatar value={agent.avatar} name={agent.name} size="lg"/><span className={agent.status === "active" ? "resident-status active" : "resident-status"}>{agent.status}</span></div><h2>{agent.name}</h2><span className="role-pill">{agent.roleName}</span><p>{agent.biography}</p><div className="resident-card-foot"><small>{agent.modelId}</small><span>View profile →</span></div></Link>)}</div>}</main>;
}
